...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template match="dri:options//dri:list">
        <xsl:choose>
            <xsl:when test="contains(@n,'account')">
                <h2 class="ds-option-set-head  h6">
                    <xsl:copy-of select="/dri:document/dri:meta/dri:userMeta/dri:metadata[@qualifier='firstName']/node()" />
                    <xsl:text> </xsl:text>
                    <xsl:copy-of select="/dri:document/dri:meta/dri:userMeta/dri:metadata[@qualifier='lastName']/node()" />
                </h2>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="dri:options/dri:list" priority="3">
        ...
        <div>
            <xsl:if test="contains(dri:list/@n,'epeople')">
                <xsl:variable name="urlReportageModule" select="confman:getProperty('dspace.module.reportage')"/>
                <a class="list-group-item ds-option" target="_blank">
                    <xsl:attribute name="href">
                        <xsl:value-of select="$urlReportageModule"/>
                        <xsl:text>?param=</xsl:text>
                        <xsl:value-of select="/dri:document/dri:meta/dri:userMeta/dri:metadata[not(@qualifier)]/node()" />
                    </xsl:attribute>
                    Módulo de Reportajes
                </a>
            </xsl:if>
    ...
    <xsl:template match="dri:options//dri:item[dri:xref]">
        <xsl:choose>
            <xsl:when test="contains(dri:xref/@target,'login') or contains(dri:xref/@target,'register')">
            </xsl:when>
            <xsl:when test="contains(dri:xref/@target,'community-list') or contains(dri:xref/@target,'community-list')">
            </xsl:when>
            <xsl:when test="contains(dri:xref/@target,'browse?type=dateissued') or contains(dri:xref/@target,'browse?type=dateissued')">
            </xsl:when>
	    <xsl:when test="contains(dri:xref/@target,'browse?type=author') or contains(dri:xref/@target,'browse?type=author')">
            </xsl:when>
	    <xsl:when test="contains(dri:xref/@target,'browse?type=function') or contains(dri:xref/@target,'browse?type=function')">
            </xsl:when>
            <xsl:otherwise>

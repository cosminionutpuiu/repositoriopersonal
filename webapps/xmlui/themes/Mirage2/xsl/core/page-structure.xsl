   ...
   <xsl:output method="xml" encoding="UTF-8" indent="yes"/>
    ...
    <!--
        ...
        directly under html body is called "ds-main". It is further subdivided into:
            "ds-header"  - the 
header div containing title, subtitle, trail and other front matter
		        ...
		        <xsl:otherwise>
                    ...
                    <xsl:if test="$submission">
            			...
            			<script type="text/javascript">
                            ...
                            // Fotógrafo
                            if ($('#aspect_submission_StepTransformer_field_dc_creator').length) {
                                var options = $('#aspect_submission_StepTransformer_field_dc_creator').val();
                                $('#aspect_submission_StepTransformer_field_dc_creator').selectize({
                                    items: options
                                });
                            }
  ...
  <script type="text/javascript">
  		...
  		document.getElementById("menu_cabe_recu").href = DSpace.context_path + "/handle/123456789/285";
  		document.getElementById("menu_cabe_expo").href = DSpace.context_path + "/handle/123456789/621";
	 	document.getElementById("menu_cabe_cont").href = DSpace.context_path + "/pages/contacta";
  		...
  		if ( window.location.pathname === DSpace.context_path + "/") {
	        ...
	        document.getElementById("menu_central_tema").href = DSpace.context_path + "/handle/123456789/482";
        	document.getElementById("menu_central_legi").href = DSpace.context_path + "/handle/123456789/481";
		...
		var urlActosInstitucionales = DSpace.context_path + '/handle/123456789/681';
    ...
    <xsl:template match="dri:body//dri:list[@id='aspect.browseArtifacts.CollectionBrowse.list.metadataTemplateList']">
        ...
        <div id="rigthMetadataDiv" class="col-sm-8">
            ...
            <!-- NUMERACIÓN FOTOGRAFÍA -->
            <xsl:if test="./dri:item/dri:field[@n='dc_description_number']">
                <div class="simple-item-view-uri item-page-field-wrapper table">
                    <h5><strong>Numeración</strong></h5>
                    <span>
                        <xsl:for-each select="./dri:item/dri:field[@n='dc_description_number']">
                            <xsl:copy-of select="./node()"/>
                            <br/>
                        </xsl:for-each>
                    </span>

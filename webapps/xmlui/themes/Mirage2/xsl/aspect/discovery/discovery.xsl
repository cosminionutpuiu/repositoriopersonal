...
<xsl:stylesheet
    ...
    exclude-result-prefixes="xalan encoder i18n dri mets dim  xlink xsl util stringescapeutils confman">
...
-->
    ...
    <xsl:template name="communitySummaryList">
	    ...
	    <!-- DESECHADO POR DEMORA EN CARGA -->
            <!-- Recuperamos la imagen del primer ítem de la coleccion -->
	    <!-- Recuperamos la url de la comunidad o coleccion -->
            <!--xsl:variable name="commCollMetadataURL">
		<xsl:value-of select="substring-before(concat('handle/', ./@n), ':collection')"/>
            </xsl:variable-->
	    <!-- Para la comunidad o coleccion recuperamos el primer item -->
	    <!--xsl:variable name="itemMetadataURL">
                <xsl:value-of select="document(concat($dspaceUrl,'/DRI/',$commCollMetadataURL))//dri:document//dri:body//dri:div//dri:div[@id='aspect.discovery.CollectionRecentSubmissions.div.collection-recent-submission']//dri:referenceSet//dri:reference[1]/@url"/>
            </xsl:variable>
	    
	    <div class="col-sm-3 hidden-xs">
            <div class="thumbnail artifact-preview">
            	<img src="{document(concat($dspaceUrl,'/DRI/',substring-before(substring-after($itemMetadataURL, '/metadata/'), '/mets.xml')))//dri:document//dri:meta//dri:pageMeta//dri:metadata[@element='citation_pdf_url']}">
    ...
    <xsl:template name="itemSummaryList">
        ...
        <div class="row ds-artifact-item ">
            ...
            <div class="col-sm-9 hidden-xs artifact-description">
                <xsl:element name="a">
		...
		<!-- VISTA DISCOVERY -->
			    ...
			    <small>
                    ...
                    <div class="abstract">
                        <xsl:for-each select="dri:list[@n=(concat($handle, ':dc.identifier'))]/dri:item">
                            <xsl:apply-templates select="."/>
                        </xsl:for-each>
                    </div>

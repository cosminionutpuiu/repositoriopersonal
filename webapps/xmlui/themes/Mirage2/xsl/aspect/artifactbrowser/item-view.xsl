...
<xsl:stylesheet
        ...
        <div class="item-summary-view-metadata">
            ...
            <div class="row">
                <div class="col-sm-4">
                    ...
                    <xsl:call-template name="itemSummaryView-DIM-descriptores"/>
                ...
                <div class="col-sm-8">
                    ...
                    <xsl:call-template name="itemSummaryView-DIM-legislatura" />
                    <xsl:call-template name="itemSummaryView-DIM-lugar" />
                    <xsl:call-template name="itemSummaryView-DIM-photo-lugar" />
                    <xsl:call-template name="itemSummaryView-DIM-authors"/>
                    <xsl:call-template name="itemSummaryView-DIM-original-format"/>
                    <xsl:call-template name="itemSummaryView-DIM-digital-format"/>
                    <xsl:call-template name="itemSummaryView-DIM-digital-signature"/>
                    <xsl:call-template name="itemSummaryView-DIM-photo-number"/>
                    <xsl:call-template name="itemSummaryView-DIM-photo-technique"/>
            ...
            <xsl:choose>
                <xsl:when test="//mets:fileSec/mets:fileGrp[@USE='THUMBNAIL']">
                    <xsl:variable name="src">
                        <xsl:choose>
                            <xsl:when test="/mets:METS/mets:fileSec/mets:fileGrp[@USE='THUMBNAIL']/mets:file[
                                    @GROUPID=../../mets:fileGrp[@USE='CONTENT']/mets:file[@GROUPID=../../mets:fileGrp[@USE='THUMBNAIL']/mets:file/@GROUPID][1]/@GROUPID]">
                                <xsl:value-of
                        ...
                        <xsl:otherwise>
        			        <a target="_blank">
                                <xsl:attribute name="href">
                                    <xsl:value-of select="//mets:fileSec/mets:fileGrp[@USE='CONTENT' or @USE='ORIGINAL' or @USE='LICENSE']/mets:file/mets:FLocat[@LOCTYPE='URL' and contains(@xlink:href, 'isAllowed=y')]/@xlink:href"/>
                                </xsl:attribute>
                    ...
                    <div id="abstract-halftwo" style="display: block">
                        <xsl:choose>
                            <xsl:when test="dim:field[@element='description'][@qualifier='abstract']">
                                <xsl:choose>
                                    <xsl:when test="string-length(dim:field[@element='description'][@qualifier='abstract'][1]/node()) &gt; 450">
                                        <xsl:copy-of select="concat(substring(dim:field[@element='description'][@qualifier='abstract'][1]/node(),1,450),'...')"/>&#160;
                                    ...
                                    <xsl:otherwise>
                                        <xsl:copy-of select="dim:field[@element='description'][@qualifier='abstract'][1]/node()"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:when>
                        </xsl:choose>
                    </div>
                    <xsl:if test="string-length(dim:field[@element='description'][@qualifier='abstract'][1]/node()) &gt; 450">
    ...
    <xsl:template name="itemSummaryView-DIM-authors">
        <xsl:if test="dim:field[@element='contributor'][@qualifier='author' and descendant::text()] or dim:field[@element='creator' and descendant::text()] or dim:field[@element='contributor' and descendant::text()]">
            <div class="simple-item-view-authors item-page-field-wrapper table">
                <h5><strong><i18n:text>xmlui.dri2xhtml.METS-1.0.item-author</i18n:text></strong></h5>
                <xsl:choose>
    ...
    <xsl:template name="itemSummaryView-DIM-URI">
        <xsl:if test="not(dim:field[@element='description' and @qualifier='number'])">
            <xsl:if test="dim:field[@element='identifier' and @qualifier='uri' and descendant::text()]">
                <div class="simple-item-view-uri item-page-field-wrapper table">
                    <h5><strong><i18n:text>Código de referencia</i18n:text></strong></h5>
                    <span>
    ...
    <xsl:template name="itemSummaryView-DIM-lugar">
        <xsl:if test="dim:field[@element='coverage' and @qualifier='geographic'] or dim:field[@element='coverage' and @qualifier='physicalplace']">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <h5><strong>Lugar</strong></h5>
            <span>
                <xsl:for-each select="dim:field[@element='coverage' and @qualifier='geographic']">
                    ...
                    <xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='geographic']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
                <xsl:if test="dim:field[@element='coverage' and @qualifier='geographic'] and dim:field[@element='coverage' and @qualifier='physicalplace']">
                    <br />
                </xsl:if>
                <xsl:for-each select="dim:field[@element='coverage' and @qualifier='physicalplace']">
                    <xsl:variable name="coverageText">
                        <xsl:copy-of select="./node()" />
                    </xsl:variable>
                    <a href="{concat(concat($context-path, '/browse?value='), concat(translate(url:encode($coverageText, 'UTF-8'), ' ', '+'), '&amp;type=coverage'))}" target="_blank">
                    ...
                    <xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='physicalplace']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <xsl:template name="itemSummaryView-DIM-photo-lugar">
        <xsl:if test="dim:field[@element='coverage' and not(@qualifier)]">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <h5><strong>Lugar</strong></h5>
            <span>
                <xsl:for-each select="dim:field[@element='coverage' and not(@qualifier)]">
                    <xsl:copy-of select="./node()"/>
                    <xsl:if test="count(following-sibling::dim:field[@element='coverage' and not(@qualifier)]) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <xsl:template name="itemSummaryView-DIM-legislatura">
        <xsl:if test="dim:field[@element='coverage' and @qualifier='legislatura']">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <span>
                <xsl:for-each select="dim:field[@element='coverage' and @qualifier='legislatura']">
                    <xsl:copy-of select="./node()"/>
                    <xsl:if test="count(following-sibling::dim:field[@element='coverage' and @qualifier='legislatura']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <xsl:template name="itemSummaryView-DIM-original-format">
        <xsl:if test="dim:field[@element='format' and @qualifier='origin']">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <h5><strong>Formato original</strong></h5>
            <span>
                <xsl:for-each select="dim:field[@element='format' and @qualifier='origin']">
                    <xsl:copy-of select="./node()"/>
                    <xsl:if test="count(following-sibling::dim:field[@element='format' and @qualifier='origin']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <xsl:template name="itemSummaryView-DIM-digital-format">
        <!-- <xsl:if test="dim:field[@element='format' and @qualifier='digital']">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <h5><strong>Formato digitalizado</strong></h5>
            <span>
                <xsl:for-each select="dim:field[@element='format' and @qualifier='digital']">
                    <xsl:copy-of select="./node()"/>
                    <xsl:if test="count(following-sibling::dim:field[@element='format' and @qualifier='digital']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <!--xsl:template name="itemSummaryView-DIM-signature">
        <xsl:if test="dim:field[@element='source' and @qualifier='signature']">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <h5><strong>Signatura</strong></h5>
            <span>
                <xsl:for-each select="dim:field[@element='source' and @qualifier='signature']">
                    <xsl:copy-of select="./node()"/>
                    <xsl:if test="count(following-sibling::dim:field[@element='source' and @qualifier='signature']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <xsl:template name="itemSummaryView-DIM-digital-signature">
    ...
    <xsl:template name="itemSummaryView-DIM-photo-number">
        <xsl:if test="dim:field[@element='description' and @qualifier='number']">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <h5><strong>Numeración</strong></h5>
            <span>
                <xsl:for-each select="dim:field[@element='description' and @qualifier='number']">
                    <xsl:copy-of select="./node()"/>
                    <xsl:if test="count(following-sibling::dim:field[@element='description' and @qualifier='number']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <xsl:template name="itemSummaryView-DIM-photo-technique">
        <xsl:if test="dim:field[@element='description' and @qualifier='phototech']">
        <div class="simple-item-view-uri item-page-field-wrapper table">
            <h5><strong>Técnica usada</strong></h5>
            <span>
                <xsl:for-each select="dim:field[@element='description' and @qualifier='phototech']">
                    <xsl:copy-of select="./node()"/>
                    <xsl:if test="count(following-sibling::dim:field[@element='description' and @qualifier='phototech']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </span>
    ...
    <xsl:template name="itemSummaryView-DIM-descriptores">
        <xsl:if test="dim:field[@element='subject' and @qualifier='function'] or dim:field[@element='contributor' and @qualifier='ja'] or dim:field[@element='contributor' and @qualifier='extern'] or dim:field[@element='contributor' and @qualifier='institution'] or dim:field[@element='subject' and not(@qualifier)]">
            <h5><strong>Descriptores: </strong></h5>
        </xsl:if>
        <xsl:if test="dim:field[@element='subject' and @qualifier='function']">
            <div class="simple-item-view-uri item-page-field-wrapper table">
                <h5><strong>Temático funcional</strong></h5>
                <xsl:for-each select="dim:field[@element='subject' and @qualifier='function']">
                    ...
                    <xsl:if test="count(following-sibling::dim:field[@element='subject' and @qualifier='function']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </div>
        </xsl:if>
        <xsl:if test="dim:field[@element='contributor' and @qualifier='ja'] or dim:field[@element='contributor' and @qualifier='extern']">
            <div class="simple-item-view-uri item-page-field-wrapper table">
                <h5><strong>Personas</strong></h5>
                <xsl:for-each select="dim:field[@element='contributor' and @qualifier='ja']">
                    ...
                    <xsl:if test="count(following-sibling::dim:field[@element='contributor' and @qualifier='ja']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
                <xsl:if test="dim:field[@element='contributor' and @qualifier='ja'] and dim:field[@element='contributor' and @qualifier='extern']">
                    <br />
                </xsl:if>
                <xsl:for-each select="dim:field[@element='contributor' and @qualifier='extern']">
                    <xsl:variable name="contributorText">
                        <xsl:copy-of select="./node()" />
                    </xsl:variable>
                    <a href="{concat(concat($context-path, '/browse?value='), concat(translate(url:encode($contributorText, 'UTF-8'), ' ', '+'), '&amp;type=contributor'))}" target="_blank">
                    ...
                    <xsl:if test="count(following-sibling::dim:field[@element='contributor' and @qualifier='extern']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </div>
        ...
        <xsl:if test="dim:field[@element='contributor' and @qualifier='institution']">
            <div class="simple-item-view-uri item-page-field-wrapper table">
                ...
                <xsl:for-each select="dim:field[@element='contributor' and @qualifier='institution']">
                    ...
                    <xsl:if test="count(following-sibling::dim:field[@element='contributor' and @qualifier='institution']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
            </div>
    ...
    <xsl:template name="itemSummaryView-show-full">
        <xsl:if test="not(dim:field[@element='description' and @qualifier='number'])">
            <div class="simple-item-view-show-full item-page-field-wrapper table">
                <h5><strong><i18n:text>xmlui.mirage2.itemSummaryView.MetaData</i18n:text></strong></h5>
                <a>
        ...
        <xsl:if test="not(dim:field[@element='description' and @qualifier='number'])">
            <xsl:if test="$document//dri:referenceSet[@id='aspect.artifactbrowser.ItemViewer.referenceSet.collection-viewer']">
                <div class="simple-item-view-collections item-page-field-wrapper table">
                    <h5>
                        <strong>Reportajes en los que aparece</strong>
                    </h5>
    ...
    <xsl:template match="dim:field" mode="itemDetailView-DIM">
            ...
            <td class="word-break">
		<xsl:if test="./@mdschema = 'dc' and ./@element='identifier' and not(./@qualifier)">
			<xsl:value-of select="confman:getProperty('dspace.metadata.prefijo.signatura')" />
		</xsl:if>
              <xsl:copy-of select="./node()"/>
            </td>

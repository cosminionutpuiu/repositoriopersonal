    ...
    <xsl:template match="dim:dim" mode="collectionDetailView-DIM">
        ...
        <xsl:if test="string-length(dim:field[@element='format'][@qualifier='extent'][1])&gt;0">
    	    ...
    	    <div class="ds-form-item row">
                <div class="control-group col-sm-12" id="nimagenes">
                	<label class="ds-form-label" for="aspect_browseArtifacts_CollectionBrowse_field_nimagenes">
    					<xsl:text>nº de imágenes: </xsl:text>
    		    	</label>
    		    	<input id="aspect_browseArtifacts_CollectionBrowse_field_nimagenes" class="ds-text-field form-control" disabled="disabled" name="dc_nimagenes" type="text">
